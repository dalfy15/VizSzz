...
type
  ...
  TReadCPChar = procedure(const ReadCPChar: TReadCPCharObj;
    var Text: ansistring; var _eof: boolean);
  ...
  public
    ...
    procedure Add(const AttrName: ansistring; const Value: ansistring;
      TestMatch: boolean = True); overload;
    procedure Add(const Attr: TZAttrArray; TestMatch: boolean = True); overload;
    procedure Add(Att: array of TZAttrArray;
      TestMatch: boolean {$IFDEF VER130} {$ELSE} = True {$ENDIF}); overload;
    procedure Assign(Source: TPersistent); override;
    procedure Clear();
    procedure DeleteItem(Index: integer);
    procedure Insert(Index: integer; const AttrName: ansistring;
      const Value: ansistring; TestMatch: boolean = True); overload;
    procedure Insert(Index: integer; const Attr: TZAttrArray;
      TestMatch: boolean = True); overload;
    function ToString(quote: ansichar; CheckEntity: boolean;
      addempty: boolean): ansistring; {$IFDEF DELPHI_UNICODE} reintroduce;
 {$ENDIF} overload; virtual;
    function ToString(quote: ansichar; CheckEntity: boolean): ansistring;
 {$IFDEF DELPHI_UNICODE} reintroduce; {$ENDIF} overload; virtual;
    function ToString(quote: ansichar): ansistring;
 {$IFDEF DELPHI_UNICODE} reintroduce; {$ENDIF} overload; virtual;
    function ToString(CheckEntity: boolean): ansistring;
 {$IFDEF DELPHI_UNICODE} reintroduce; {$ENDIF} overload; virtual;
    function ToString(): ansistring; {$IFDEF DELPHI_UNICODE} reintroduce;
 {$ENDIF} overload;  {$IFDEF Z_FPC_USE_TOSTRING} override; {$ELSE} virtual; {$ENDIF}
    function IsContainsAttribute(const AttrName: ansistring;
      CaseSensitivity: boolean = True): boolean;
    property Count: integer read FCount;
    property Items[num: integer]: TZAttrArray read GetAttr write SetAttr;
    property ItemsByName[const Att: ansistring]: ansistring read GetAttrS write SetAttrS;
      default;
  ...
  private
    ...
    //какие кавычки используем для атрибутов
  ...
  protected
    ...
    procedure _AddTag(const _begin: ansistring; Text: ansistring;
      const _end: ansistring; StartNewLine: boolean; _tab: integer = 0);
  ...
  public
    ...
    //начать запись в Buffer
    procedure EndSaveTo();
    procedure FlushBuffer();
    procedure WriteCDATA(CDATA: ansistring; CorrectCDATA: boolean;
      StartNewLine: boolean = True); overload;     //<![CDATA[ bla-bla-bla <><><>...]]>
    ...
    //<!-- bla-bla-bla -->
    procedure WriteEmptyTag(const TagName: ansistring; SAttributes: TZAttributes;
      StartNewLine: boolean; CheckEntity: boolean = True); overload; // <tag a="a"... />
    procedure WriteEmptyTag(const TagName: ansistring;
      AttrArray: array of TZAttrArray; StartNewLine: boolean;
      CheckEntity: boolean = True); overload;
    procedure WriteEmptyTag(const TagName: ansistring; StartNewLine: boolean;
      CheckEntity: boolean = True); overload;
    procedure WriteEmptyTag(const TagName: ansistring; SAttributes: TZAttributes);
      overload;
    procedure WriteEmptyTag(const TagName: ansistring;
      AttrArray: array of TZAttrArray); overload;
    ...
    procedure WriteInstruction(const InstructionName: ansistring;
      SAttributes: TZAttributes; StartNewLine: boolean; CheckEntity: boolean = True);
      overload;
    procedure WriteInstruction(const InstructionName: ansistring;
      AttrArray: array of TZAttrArray; StartNewLine: boolean;
      CheckEntity: boolean = True); overload;
    ...
    procedure WriteInstruction(const InstructionName: ansistring;
      SAttributes: TZAttributes); overload;
    procedure WriteInstruction(const InstructionName: ansistring;
      AttrArray: array of TZAttrArray); overload;
    procedure WriteInstruction(const InstructionName: ansistring); overload;
    procedure WriteRaw(Text: ansistring; UseConverter: boolean;
      StartNewLine: boolean = True);
    procedure WriteTag(const TagName: ansistring; const Text: ansistring;
      SAttributes: TZAttributes; StartNewLine: boolean; CloseTagNewLine: boolean;
      CheckEntity: boolean = True); overload;
    procedure WriteTag(const TagName: ansistring; const Text: ansistring;
      AttrArray: array of TZAttrArray; StartNewLine: boolean;
    ...
    procedure WriteTagNode(const TagName: ansistring; SAttributes: TZAttributes;
      StartNewLine: boolean; CloseTagNewLine: boolean; CheckEntity: boolean = True);
      overload;
    procedure WriteTagNode(const TagName: ansistring; AttrArray: array of TZAttrArray;
      StartNewLine: boolean; CloseTagNewLine: boolean;
      CheckEntity: boolean {$IFDEF VER130} {$ELSE} = True{$ENDIF}); overload;
    procedure WriteTagNode(const TagName: ansistring; StartNewLine: boolean;
      CloseTagNewLine: boolean; CheckEntity: boolean = True); overload;
    procedure WriteTagNode(const TagName: ansistring; SAttributes: TZAttributes);
      overload;
    procedure WriteTagNode(const TagName: ansistring; AttrArray: array of TZAttrArray);
    ...
    property TextConverter: TAnsiToCPConverter
      read FTextConverter write SetTextConverter;
  ...
  private
    ...
    FTagName:    ansistring;
    //Имя тэга (инструкции/комментария)
    FValue:      ansistring;
    //Текст CDATA или комментария
  ...
  public
    ...
    function BeginReadString(Source: ansistring;
      IgnoreCodePage: boolean = True): integer;
  ...
  public
    constructor Create();
    destructor Destroy(); override;
    procedure Add(const AttrName: string; const Value: string;
      TestMatch: boolean = True); overload;
    ...
    procedure Clear();
    procedure DeleteItem(Index: integer);
    procedure Insert(Index: integer; const AttrName: string;
      const Value: string; TestMatch: boolean = True); overload;
    procedure Insert(Index: integer; const Attr: TZAttrArrayH;
      TestMatch: boolean = True); overload;
    function ToString(quote: char; CheckEntity: boolean; addempty: boolean): string;
      reintroduce; overload; virtual;
    function ToString(quote: char; CheckEntity: boolean): string;
      reintroduce; overload; virtual;
    ...
    function IsContainsAttribute(const AttrName: string;
      CaseSensitivity: boolean = True): boolean;
  ...
  public
    ...
    procedure EndSaveTo();
    procedure FlushBuffer();
    procedure WriteCDATA(CDATA: string; CorrectCDATA: boolean;
      StartNewLine: boolean = True); overload;     //<![CDATA[ bla-bla-bla <><><>...]]>
    procedure WriteCDATA(CDATA: string); overload;
    procedure WriteComment(Comment: string; StartNewLine: boolean = True);
    //<!-- bla-bla-bla -->
    procedure WriteEmptyTag(TagName: string; SAttributes: TZAttributesH;
      StartNewLine: boolean; CheckEntity: boolean = True); overload; // <tag a="a"... />
    procedure WriteEmptyTag(TagName: string; AttrArray: array of TZAttrArrayH;
      StartNewLine: boolean; CheckEntity: boolean = True); overload;
    procedure WriteEmptyTag(TagName: string; StartNewLine: boolean;
      CheckEntity: boolean = True); overload;
    ...
    procedure WriteEndTagNode(); overload;
    procedure WriteEndTagNode(isForce: boolean; CloseTagNewLine: boolean); overload;
    procedure WriteInstruction(InstructionName: string; SAttributes: TZAttributesH;
      StartNewLine: boolean; CheckEntity: boolean = True); overload;
    procedure WriteInstruction(InstructionName: string;
      AttrArray: array of TZAttrArrayH; StartNewLine: boolean;
      CheckEntity: boolean = True); overload;
    procedure WriteInstruction(InstructionName: string; StartNewLine: boolean;
      CheckEntity: boolean = True); overload;
    procedure WriteInstruction(InstructionName: string; SAttributes: TZAttributesH);
      overload;
    procedure WriteInstruction(InstructionName: string;
      AttrArray: array of TZAttrArrayH); overload;
    procedure WriteInstruction(InstructionName: string); overload;
    procedure WriteRaw(Text: string; UseConverter: boolean;
      StartNewLine: boolean = True); overload;
    procedure WriteRaw(Text: ansistring; UseConverter: boolean;
      StartNewLine: boolean = True); overload;
    procedure WriteTag(TagName: string; Text: string; SAttributes: TZAttributesH;
      StartNewLine: boolean; CloseTagNewLine: boolean; CheckEntity: boolean = True);
      overload;
    procedure WriteTag(TagName: string; Text: string; AttrArray: array of TZAttrArrayH;
      StartNewLine: boolean; CloseTagNewLine: boolean; CheckEntity: boolean = True);
      overload;
    procedure WriteTag(TagName: string; Text: string; StartNewLine: boolean;
      CloseTagNewLine: boolean; CheckEntity: boolean = True); overload;
    procedure WriteTag(TagName: string; Text: string; SAttributes: TZAttributesH);
      overload;
    procedure WriteTag(TagName: string; Text: string; AttrArray: array of TZAttrArrayH);
    ...
    procedure WriteTagNode(TagName: string; SAttributes: TZAttributesH;
      StartNewLine: boolean; CloseTagNewLine: boolean; CheckEntity: boolean = True);
      overload;
    procedure WriteTagNode(TagName: string; AttrArray: array of TZAttrArrayH;
      StartNewLine: boolean; CloseTagNewLine: boolean; CheckEntity: boolean = True);
      overload;
    procedure WriteTagNode(TagName: string; StartNewLine: boolean;
      CloseTagNewLine: boolean; CheckEntity: boolean = True); overload;
    ...
    property TextConverter: TAnsiToCPConverter
      read GetTextConverter write SetTextConverter;
...
//Читатали
procedure ReadCharUTF8(const ReadCPChar: TReadCPCharObj; var Text: ansistring;
  var _eof: boolean);
procedure ReadCharUTF16LE(const ReadCPChar: TReadCPCharObj;
  var Text: ansistring; var _eof: boolean);
procedure ReadCharUTF16BE(const ReadCPChar: TReadCPCharObj;
  var Text: ansistring; var _eof: boolean);
procedure ReadCharUTF32(const ReadCPChar: TReadCPCharObj; var Text: ansistring;
  var _eof: boolean);
procedure ReadCharOneByte(const ReadCPChar: TReadCPCharObj;
  var Text: ansistring; var _eof: boolean);
...
function RecognizeEncodingXML(startpos: integer; var txt: ansistring;
  out cpfromtext: integer; out cpname: ansistring; out ftype: integer): boolean;
  overload;
...
function RecognizeEncodingXML(var txt: ansistring; out BOM: integer;
  out cpfromtext: integer; out cpname: ansistring; out ftype: integer): boolean;
  overload;
      ...
      //точно UTF32LE
        ...
        //точно не UTF-16/32
          if (kol16BE = 0) and (kol16LE = 0) and (kol32BE = 0) and (kol32LE = 0) then
            ...
            begin
            Result := False;
            // точно не понятно что за кодировка
        ...
        if txt[2] = #254 then
          ...
          if ltxt >= 4 then
            if (txt[3] = #0) and (txt[4] = #0) then
              Result :=
                5//не учитываем, что текст в UTF-16LE может начинаться с 00 00
          ;
          end;
      #0:   //UTF-32BE?
        if ltxt >= 4 then
...
procedure TZAttributes.Insert(Index: integer; const AttrName: ansistring;
  const Value: ansistring; TestMatch: boolean = True);
var
  i: integer;
...
procedure TZAttributes.Insert(Index: integer; const Attr: TZAttrArray;
...
procedure TZAttributes.Add(const AttrName: ansistring; const Value: ansistring;
  TestMatch: boolean = True);
var
  i: integer;
...
//           TestMatch: boolean - если true, проверяет на AttrName на совпадение с ранее
//                                введёнными, если совпадает, меняет значение, иначе добавляет
procedure TZAttributes.Add(Att: array of TZAttrArray;
  TestMatch: boolean {$IFDEF VER130} {$ELSE} = True {$ENDIF});
var
  i: integer;
...
function TZAttributes.ToString(quote: ansichar; CheckEntity: boolean;
  addempty: boolean): ansistring;
var
  i: integer;
  ...
  if CheckEntity then
    ...
    for i := 0 to Count - 1 do
      if (length(FItems[i][1]) > 0) or (addempty) then
        Result := Result + ' ' + FItems[i][0] + '=' + quote +
          CheckStrEntity(FItems[i][1]) + quote;
    end
  else
    for i := 0 to Count - 1 do
      if (length(FItems[i][1]) > 0) or (addempty) then
...
function TZAttributes.IsContainsAttribute(const AttrName: ansistring;
  CaseSensitivity: boolean = True): boolean;
var
  i: integer;
  s: string;
...
procedure TZsspXMLWriter._AddTag(const _begin: ansistring; Text: ansistring;
  const _end: ansistring; StartNewLine: boolean; _tab: integer = 0);
begin
...
//                                   если NewLine = false)
procedure TZsspXMLWriter.WriteCDATA(CDATA: ansistring; CorrectCDATA: boolean;
  StartNewLine: boolean = True);
var
  p: integer;
...
//           StartNewLine: boolean  - начинать с новой строки (игнорируется,
//                                   если NewLine = false)
procedure TZsspXMLWriter.WriteComment(const Comment: ansistring;
  StartNewLine: boolean = True);
begin
...
//           StartNewLine: boolean  - начинать с новой строки (игнорируется,
//                                   если NewLine = false)
procedure TZsspXMLWriter.WriteRaw(Text: ansistring; UseConverter: boolean;
  StartNewLine: boolean = True);
begin
  if not FInProcess then
    exit;
...
procedure TZsspXMLWriter.WriteTag(const TagName: ansistring;
  const Text: ansistring; AttrArray: array of TZAttrArray; StartNewLine: boolean;
...
//           CheckEntity: boolean       - проверять и корректировать спецсимволы
procedure TZsspXMLWriter.WriteTag(const TagName: ansistring;
  const Text: ansistring; SAttributes: TZAttributes; StartNewLine: boolean;
  CloseTagNewLine: boolean; CheckEntity: boolean = True);
var
  s: ansistring;
...
//           CheckEntity: boolean       - проверять и корректировать спецсимволы
procedure TZsspXMLWriter.WriteTag(const TagName: ansistring;
  const Text: ansistring; StartNewLine: boolean; CloseTagNewLine: boolean;
...
begin
 ...
 //           SAttributes: TZAttributes   - атрибуты
procedure TZsspXMLWriter.WriteTag(const TagName: ansistring;
  const Text: ansistring; SAttributes: TZAttributes);
begin
 ...
 //           AttrArray: array of TZAttrArray   - атрибуты
procedure TZsspXMLWriter.WriteTag(const TagName: ansistring;
  const Text: ansistring; AttrArray: array of TZAttrArray);
var
  t: TZAttributes;
...
//           CheckEntity: boolean       - проверять и корректировать спецсимволы
procedure TZsspXMLWriter.WriteTagNode(const TagName: ansistring;
  AttrArray: array of TZAttrArray; StartNewLine: boolean; CloseTagNewLine: boolean;
  CheckEntity: boolean{$IFDEF VER130}{$ELSE} = True{$ENDIF} );
var
  t: TZAttributes;
...
begin
  ...
  if TagCount > 0 then
    begin
    _AddTag('</', FTags[TagCount - 1].Name, '>', FTags[TagCount -
      1].CloseTagNewLine, -1);
    Dec(FTagCount);
...
procedure TZsspXMLWriter.WriteEmptyTag(const TagName: ansistring;
  SAttributes: TZAttributes; StartNewLine: boolean; CheckEntity: boolean = True);
begin
  _AddTag('<' + TagName, SAttributes.ToString(AttributeQuote, CheckEntity), '/>',
    StartNewLine);
end;
...
//           CheckEntity: boolean       - проверять и корректировать спецсимволы
procedure TZsspXMLWriter.WriteEmptyTag(const TagName: ansistring;
  AttrArray: array of TZAttrArray; StartNewLine: boolean; CheckEntity: boolean = True);
var
  t: TZAttributes;
...
procedure TZsspXMLWriter.WriteInstruction(const InstructionName: ansistring;
  SAttributes: TZAttributes; StartNewLine: boolean; CheckEntity: boolean = True);
begin
  _AddTag('<?' + InstructionName, SAttributes.ToString(AttributeQuote,
    CheckEntity), '?>',
    StartNewLine);
end;
...
//      IgnoreCodePage: boolean - Игнорировать ли кодировку текста
 ...
 //                      2 - какое-то ошибко
 //                      3 - Stream = nil
function TZsspXMLReader.BeginReadString(Source: ansistring;
  IgnoreCodePage: boolean = True): integer;
begin
  if InProcess then
    Result := 1
...
{$ENDIF}
  ...
  begin
    ...
    if ss[1] <> #0 then
      //todo: что делать, если в тексте действительно будет #0?
        ...
        case ss[1] of
          ...
          '=':
            ...
            if _isTagName > 0 then
              ...
              if length(s) > 0 then
                ...
                s := '';
                GetParamValue();
                //может, забыли имя параметра ввести? {tut}
                end;
              FErrorCode := FErrorCode or 256;
              _isTagName := 13;
              end;
          ...
          '!':  //комментарий / CDATA
            if (_isTagName = 0) and (length(s) = 0) then
              ...
              FErrorCode := FErrorCode or 512// <TAG!>
            ;
          '?':
            if (_isTagName = 0) and (length(s) = 0) then
              FTagType := 1
          ...
          '/':
            if (_isTagName = 0) and (length(s) = 0) then
              ...
              if _isTagName = 0 then
                FTagName   := s;
              _isClosedTag := True;
              end;// сделать проверку, если текст тэга не пустой
...
begin
  ...
  if IgnoreCase then
    begin
    {$IFDEF DELPHI_UNICODE}
    if (AnsiUpperCase(UTF8ToString(TagName)) =
      AnsiUpperCase(UTF8ToString(Tags[TagCount - 1]))) then
    {$ELSE}
...
procedure TZAttributesH.Add(const AttrName: string; const Value: string;
  TestMatch: boolean = True);
begin
...
procedure TZAttributesH.Insert(Index: integer; const AttrName: string;
  const Value: string; TestMatch: boolean = True);
begin
  FAttributes.Insert(Index, UTF8Encode(AttrName), UTF8Encode(Value), TestMatch);
end;

procedure TZAttributesH.Insert(Index: integer; const Attr: TZAttrArrayH;
  TestMatch: boolean = True);
begin
  Insert(Index, Attr[0], Attr[1], TestMatch);
end;

function TZAttributesH.ToString(quote: char; CheckEntity: boolean;
  addempty: boolean): string;
begin
...
function TZAttributesH.IsContainsAttribute(const AttrName: string;
  CaseSensitivity: boolean = True): boolean;
begin
...
procedure TZsspXMLWriterH.WriteCDATA(CDATA: string; CorrectCDATA: boolean;
  StartNewLine: boolean = True);
begin
...
procedure TZsspXMLWriterH.WriteEmptyTag(TagName: string;
  SAttributes: TZAttributesH; StartNewLine: boolean; CheckEntity: boolean = True);
begin
  FXMLWriter.WriteEmptyTag(UTF8Encode(TagName), SAttributes.FAttributes,
    StartNewLine, CheckEntity);
end;

procedure TZsspXMLWriterH.WriteEmptyTag(TagName: string;
  AttrArray: array of TZAttrArrayH; StartNewLine: boolean; CheckEntity: boolean = True);
var
...
procedure TZsspXMLWriterH.WriteEmptyTag(TagName: string; StartNewLine: boolean;
  CheckEntity: boolean = True);
begin
  FXMLWriter.WriteEmptyTag(UTF8Encode(TagName), FAttributes.FAttributes,
    StartNewLine, CheckEntity);
end;
...
procedure TZsspXMLWriterH.WriteEmptyTag(TagName: string;
  AttrArray: array of TZAttrArrayH);
begin
  WriteEmptyTag(TagName, AttrArray, True, True);
end;
...
begin
  ...
  _start := Low(AttrArray);
    ...
    FXMLWriter.WriteInstruction(UTF8Encode(InstructionName), a,
      StartNewLine, CheckEntity);
    finally
    SetLength(a, 0);
    a := nil;
...
procedure TZsspXMLWriterH.WriteRaw(Text: string; UseConverter: boolean;
  StartNewLine: boolean = True);
begin
  FXMLWriter.WriteRaw(UTF8Encode(Text), UseConverter, StartNewLine);
end;

procedure TZsspXMLWriterH.WriteRaw(Text: ansistring; UseConverter: boolean;
  StartNewLine: boolean = True);
begin
...
procedure TZsspXMLWriterH.WriteTag(TagName: string; Text: string;
  SAttributes: TZAttributesH; StartNewLine: boolean; CloseTagNewLine: boolean;
  CheckEntity: boolean = True);
begin
  FXMLWriter.WriteTag(UTF8Encode(TagName), UTF8Encode(Text),
    SAttributes.FAttributes, StartNewLine, CloseTagNewLine, CheckEntity);
end;
...
begin
  ...
  _start := Low(AttrArray);
    ...
    FXMLWriter.WriteTag(UTF8Encode(TagName), UTF8Encode(Text), a,
      StartNewLine, CloseTagNewLine, CheckEntity);
    finally
    SetLength(a, 0);
    a := nil;
    end;
...
procedure TZsspXMLWriterH.WriteTagNode(TagName: string;
  ...
  CheckEntity: boolean = True);
begin
  FXMLWriter.WriteTagNode(UTF8Encode(TagName), SAttributes.FAttributes,
    StartNewLine, CloseTagNewLine, CheckEntity);
end;
...
begin
  ...
  _start := Low(AttrArray);
    ...
    FXMLWriter.WriteTagNode(UTF8Encode(TagName), a, StartNewLine,
      CloseTagNewLine, CheckEntity);
    finally
    SetLength(a, 0);
    a := nil;
    end;
...
procedure TZsspXMLWriterH.WriteTagNode(TagName: string; StartNewLine: boolean;
  CloseTagNewLine: boolean; CheckEntity: boolean = True);
begin
...
function TZsspXMLReaderH.BeginReadString(Source: string;
  IgnoreCodePage: boolean = True): integer;
begin
